...
void Cell::updateFeq(std::vector<float>& feq, const float& uX, const float&  uY, const float& rho) { 
    ...
    for (int i=0; i<9; i++) {
        ...
        feq.at(i) = D2Q9.weights[i] * rho * (1 + temp / std::pow(CS, 2) + std::pow(temp, 2) / (2 * std::pow(CS, 4)) - uprod / (2 * std::pow(CS, 2)));
    }
}
...
void Cell::streaming(const std::vector<float> fstar, Lattice& lattice)
{
    for (int i = 0; i < 9; i++)
    {   
        Cell& newCell = lattice.getCellAtIndex({position.at(0) + D2Q9.velocities[i][0], position.at(1) + D2Q9.velocities[i][1]});
