...
//                            {0., 0., 0., 1., -1., 1., -1, 1., -1.}};
constexpr float velocities[number_of_directions][2] = {
...
void setVelocitiesAtWalls(double sigma, double u_lid, int it, float uTop[][100], float uBot[][100], float uLeft[][100],
                          float uRight[][100])
{
...
void collision_and_streaming(double om_p, double om_m, int lx, int ly, const float fEq[][100][100], float f[][100][100],
                             float fNew[][100][100])
{
    ...
    for (int i = 0; i < 9; ++i)
    {
        ...
        for (int j = 0; j < lx + 1; ++j)
        {
            ...
            {
                fNew[i][j][k] = (1.0 - 0.5 * (om_p + om_m)) * f[i][j][k] - 0.5 * (om_p - om_m) * f[qb][j][k] +
                                0.5 * (om_m + om_p) * fEq[i][j][k] + 0.5 * (om_m - om_p) * fEq[qb][j][k];
            }
        }
    }
...
void zou_he_bottom_left_corner_velocity(float f[][100][100], float u[][100][100], float rho[][100], int ly)
{
    ...
    f[0][0][ly] = rho[0][ly] - f[1][0][ly] - f[2][0][ly] - f[3][0][ly] - f[4][0][ly] - f[5][0][ly] - f[6][0][ly] -
                  f[7][0][ly] - f[8][0][ly];
}
...
void zou_he_bottom_right_corner_velocity(float f[][100][100], float u[][100][100], float rho[][100], int lx)
{
    ...
    f[0][lx][0] = rho[9][0] - f[1][lx][0] - f[2][lx][0] - f[3][lx][0] - f[4][lx][0] - f[5][lx][0] - f[6][lx][0] -
                  f[7][lx][0] - f[8][lx][0];
}
...
void zou_he_top_right_corner_velocity(float f[][100][100], float u[][100][100], float rho[][100], int lx)
{
    ...
    f[0][lx][0] = rho[lx][0] - f[1][lx][0] - f[2][lx][0] - f[3][lx][0] - f[4][lx][0] - f[5][lx][0] - f[6][lx][0] -
                  f[7][lx][0] - f[8][lx][0];
}
...
void zou_he_left_wall_velocity(float f[][100][100], float u[][100][100], float rho[][100], int ly, float uLeft[][100])
{
    for (int i = 0; i < ly; i++)
    {
        ...
        rho[0][i] =
            (f[0][0][i] + f[2][0][i] + f[4][0][i] + 2.0 * (f[3][0][i] + f[6][0][i] + f[7][0][i])) / (1.0 - u[0][0][i]);

        f[1][0][i] = f[3][0][i] + 2.0 / 3.0 * rho[0][i] * u[0][0][i];
        f[5][0][i] = f[7][0][i] - 0.5 * (f[2][0][i] - f[4][0][i]) + 1.0 / 6.0 * rho[0][i] * u[0][0][i] +
                     0.5 * rho[0][i] * u[1][0][i];
        f[8][0][i] = f[6][0][i] + 0.5 * (f[2][0][i] - f[4][0][i]) + 1.0 / 6.0 * rho[0][i] * u[0][0][i] -
                     0.5 * rho[0][i] * u[1][0][i];
    }
}
void zou_he_right_wall_velocity(float f[][100][100], float u[][100][100], float rho[][100], int ly, float uRight[][100],
                                int lx)
{
    for (int i = 0; i < ly; i++)
    {
        ...
        rho[lx][i] = (f[0][lx][i] + f[2][lx][i] + f[4][lx][i] + 2.0 * (f[1][lx][i] + f[5][lx][i] + f[8][lx][i])) /
                     (1.0 + u[0][lx][i]);

        f[3][lx][i] = f[1][lx][i] - 2.0 / 3.0 * rho[lx][i] * u[0][lx][i];
        f[6][lx][i] = f[8][lx][i] - 0.5 * (f[2][lx][i] - f[4][lx][i]) - 1.0 / 6.0 * rho[lx][i] * u[0][lx][i] -
                      0.5 * rho[lx][i] * u[1][lx][i];
        f[7][lx][i] = f[5][lx][i] + 0.5 * (f[2][lx][i] - f[4][lx][i]) - 1.0 / 6.0 * rho[lx][i] * u[0][lx][i] -
                      0.5 * rho[lx][i] * u[1][lx][i];
    }
}
void zou_he_top_wall_velocity(float f[][100][100], float u[][100][100], float rho[][100], int ly, float uTop[][100],
                              int lx)
{
    for (int i = 0; i < lx; i++)
    {
        ...
        rho[i][0] =
            (f[0][i][0] + f[1][i][0] + f[3][i][0] + 2.0 * (f[2][i][0] + f[5][i][0] + f[6][i][0])) / (1.0 + u[1][i][0]);

        f[4][i][0] = f[2][i][0] - 2.0 / 3.0 * rho[i][0] * u[1][i][0];
        f[8][i][0] = f[6][i][0] - 0.5 * (f[1][i][0] - f[3][i][0]) - 1.0 / 6.0 * rho[i][0] * u[1][i][0] +
                     0.5 * rho[i][0] * u[0][i][0];
        f[7][i][0] = f[5][i][0] + 0.5 * (f[1][i][0] - f[3][i][0]) - 1.0 / 6.0 * rho[i][0] * u[1][i][0] -
                     0.5 * rho[i][0] * u[0][i][0];
    }
}

void zou_he_bottom_wall_velocity(float f[][100][100], float u[][100][100], float rho[][100], int ly, float uBot[][100],
                                 int lx)
{
    for (int i = 0; i < lx; i++)
    {
        ...
        rho[i][ly] = (f[0][i][ly] + f[1][i][ly] + f[3][i][ly] + 2.0 * (f[4][i][ly] + f[7][i][ly] + f[8][i][ly])) /
                     (1.0 - u[1][i][ly]);

        f[2][i][ly] = f[4][i][ly] + 2.0 / 3.0 * rho[i][ly] * u[1][i][ly];
        f[5][i][ly] = f[7][i][ly] - 0.5 * (f[1][i][ly] - f[3][i][ly]) + 1.0 / 6.0 * rho[i][ly] * u[1][i][ly] +
                      0.5 * rho[i][ly] * u[0][i][ly];
        f[6][i][ly] = f[8][i][ly] + 0.5 * (f[1][i][ly] - f[3][i][ly]) + 1.0 / 6.0 * rho[i][ly] * u[1][i][ly] -
                      0.5 * rho[i][ly] * u[0][i][ly];
    }
}
...
int main()
{
    ...
    setVelocitiesAtWalls(sigma, u_lbm, 0, uTop, uBot, uLeft,
                         uRight); // 0 perchè è la prima iterazione (in realtà inizializzazione)
    ...
    for (int it = 0; it < it_max + 1; ++it)
    {
        ...
        profile(zou_he_bottom_wall_velocity(f, u, rho, ly, uBot, lx); zou_he_left_wall_velocity(f, u, rho, ly, uLeft);
                zou_he_right_wall_velocity(f, u, rho, ly, uRight, lx);
                zou_he_top_wall_velocity(f, u, rho, ly, uTop, lx); zou_he_bottom_left_corner_velocity(f, u, rho, ly);
                zou_he_top_left_corner_velocity(f, u, rho); zou_he_top_right_corner_velocity(f, u, rho, lx);
                zou_he_bottom_right_corner_velocity(f, u, rho, lx);, "boundary_conditions");
